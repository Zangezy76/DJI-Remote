...
void sendRecordCommand(bool start) {
    ...
    Serial.printf("[TX] %s record: ",start?"START":"STOP");
...
// ═══════════════════════════════════════════════════════════
// GPS инъекция в камеру — CmdSet=0x00, CmdID=0x17
// ═══════════════════════════════════════════════════════════
void sendGpsData() {
    if (!pWriteChr || !connected) return;
    ...
    // gps_data_push_command_frame_t — 45 байт
    uint8_t payload[45];
    ...
    // speed_to_north, speed_to_east: cm/s из курса и скорости
    float speed_ms = gps.speed.isValid() ? (float)(gps.speed.mps()) : 0.0f;
    float course   = gps.course.isValid() ? (float)(gps.course.deg() * M_PI / 180.0) : 0.0f;
    float spd_north = speed_ms * cos(course) * 100.0f; // в cm/s
    float spd_east  = speed_ms * sin(course) * 100.0f;
    float spd_down  = 0.0f;
    memcpy(p, &spd_north, 4); p += 4;
    memcpy(p, &spd_east,  4); p += 4;
    memcpy(p, &spd_down,  4); p += 4;

    // accuracy fields
    float v_acc = 2.0f, h_acc = 2.0f, s_acc = 0.1f;
    memcpy(p, &v_acc, 4); p += 4;
    memcpy(p, &h_acc, 4); p += 4;
    memcpy(p, &s_acc, 4); p += 4;
    ...
    *p = (uint8_t)gps.satellites.value();
    ...
    Serial.printf("[GPS] → camera: lat=%.6f lon=%.6f alt=%.1fm sats=%d\n",
...
void bleTask(void* pvParameters) {
    ...
    if(!pClient->connect(addr)){
        ...
        statusChanged=true; vTaskDelete(NULL); return;
    }
    auto svc=pClient->getService("0000fff0-0000-1000-8000-00805f9b34fb");
    if(!svc){snprintf(statusText,sizeof(statusText),"SVC FFF0\nnot found!");statusChanged=true;pClient->disconnect();vTaskDelete(NULL);return;}
    pWriteChr=svc->getCharacteristic("0000fff3-0000-1000-8000-00805f9b34fb");
    if(!pWriteChr){snprintf(statusText,sizeof(statusText),"CHR FFF3\nnot found!");statusChanged=true;pClient->disconnect();vTaskDelete(NULL);return;}
    ...
    if(nc&&nc->canNotify()){
        nc->subscribe(true,[](NimBLERemoteCharacteristic* c,uint8_t* data,size_t len,bool isNotify){
            Serial.printf("[RX] %zu: %02X %02X %02X %02X\n",len,len>0?data[0]:0,len>1?data[1]:0,len>2?data[2]:0,len>3?data[3]:0);
        });
    }
    connected=true;
...
void setup(){
    ...
    // GPS UART
    gpsSerial.begin(9600, SERIAL_8N1, 43, 44); // RX=43, TX=44
    ...
    lv_obj_align(lblStatus,LV_ALIGN_TOP_MID,0,20);

    lblInfo=lv_label_create(lv_scr_act());
    lv_label_set_text(lblInfo,"GPS: searching...");
    ...
    lv_obj_align(lblInfo,LV_ALIGN_TOP_MID,0,150);
...
void loop(){
    ...
    // Обновление UI с GPS данными каждые 2 сек
    ...
    if(millis()-lastUiUpdate > 2000){
        ...
        if(gps.location.isValid()){
            snprintf(infoText,sizeof(infoText),
                ...
                (connected&&gps.satellites.value()>0) ? "ON" : "OFF");
        } else {
            snprintf(infoText,sizeof(infoText),"GPS: searching...\nSats: %d\nChars: %lu",
                (int)gps.satellites.value(), gps.charsProcessed());
        }
        lv_label_set_text(lblInfo,infoText);
    }

    // GPS инъекция каждую секунду
